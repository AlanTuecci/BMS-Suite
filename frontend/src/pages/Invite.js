...
const Invite = () => {
  ...
  if (loading) return <p className="text-center text-gray-600">Loading invites...</p>;

  return (
    <div className="flex h-screen bg-white">
      <Sidebar />
      <div className="flex-grow p-8 ml-16">
        <h1 className="text-5xl font-light text-gray-800 mb-4 mt-4 leading-tight">Invite Users</h1>
        <p className="text-gray-600 mb-6">Invite new employees to join your company.</p>
        <div className="mb-6">
          ...
          <form onSubmit={handleInvite} className="flex">
            <input
              ...
              required
            />
            <button type="submit" className="px-6 py-2 bg-compblue text-white rounded-r-md hover:bg-lighter_purple">
              Send Invite
            </button>
          </form>
          {errors.employee_email && <div className="text-red-600 mt-2">{errors.employee_email}</div>}
